...
1. **Freeze the Quest:**
   ```
   python3 .court/engine/cli.py pillory <id> --reason "<one-line reason>" --decrees "<numbered decrees, one line each, semicolon-separated>" [--successor <new_id>]
   ```
...
2. **Charter the Successor (Steward's follow-through):**
   ...
   - Wire the pair if `--successor` was not passed at pillory time:
     ```
     python3 .court/engine/cli.py set-field <punished_id> pilloried_by <successor_id>
     python3 .court/engine/cli.py set-field <successor_id> pillory_of <punished_id>
     ```
