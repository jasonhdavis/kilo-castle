...
1. Identify the target Quest(s):
   ...
   - If no arguments are provided, target all active Quests currently in `WORKING` stage with an active Serf session (`python3 -m court.cli status`).
...
2. For each target Quest:
   ...
   - Record the rendered Tribute into the Quest file:
     ```
     python3 -m court.cli set-section <id> "Tribute Rendered" --file <report_temp_file>
     ```
