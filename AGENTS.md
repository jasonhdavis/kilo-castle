...
- **Steward on `castle`**: Manages lifecycle, planning, Council (`/plot`), dispatching, triage, and teardowns. Does not run test suites directly on `castle`. Model: **Gemini 3.7 Flash** (`openrouter/google/gemini-3.7-flash`).
...
1. **Gate 1 — Creation & Spawning**:
   - Stand up Serf session via Court CLI (`court dispatch <id>`) or Kilo CLI (`kilo run --agent serf --model openrouter/z-ai/glm-5.3-flash --dir <wt>`).
