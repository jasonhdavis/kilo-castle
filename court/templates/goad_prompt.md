...
3. `python3 .court/engine/cli.py show <QUEST_ID>` — read `# Expected Tribute` (checklist),
   `# Tribute Rendered` (narrative writeup), and the current `status:` frontmatter field. This
   is the ground truth, not chat memory.
...
- **Case A — Tasks checklist is 100% checked off, working tree is clean, but `# Tribute
  ...
  2. Write the full `# Tribute Rendered` section (Ballad, Tribute, Penance, Audience,
     ...
     `python3 .court/engine/cli.py set-section <QUEST_ID> "Tribute Rendered" --content "<full markdown>"`
