...
## Division of Labor & Kilo CLI Preferred Path

> **Canonical Execution Policy**: The **Kilo CLI (`kilo run --agent <role>`) is the mandatory execution path** for all autonomous roles (Serfs, Scouts, Master of Coin, Gatekeeper). Agent Manager in the VS Code extension UI suffers from workspace-root context caching where sessions default to `steward` mode and lack agent parameterization. Agent Manager in VS Code is reserved strictly for human viewing, terminal monitoring, and visual diff inspection.

- **Serf on Quest Worktrees (Never Steward)**: Quests MUST ALWAYS be spawned and executed using a **Serf** worker with **GLM 5.3 Flash** via Kilo CLI (`kilo run --agent serf --model "$(python3 -m court.cli model serf)" --dir <wt>` or `python3 -m court.cli dispatch <id>`), **NEVER via Agent Manager UI or tool**. The Steward resides on `castle`. Worktrees belong strictly to disposable Serf workers who implement the assigned Goal & Scope without orchestration authority or subagent permissions (`task: deny`).
- **Master of Coin (Dedicated Session via Kilo CLI; Broad Authority, Narrow Remit)**: The Master of Coin is the Court's administrative/accounting arm. It MUST ALWAYS evaluate inside the Quest's existing worktree/branch via Kilo CLI (`python3 -m court.cli coin <id>` or `kilo run --agent master_of_coin --model "$(python3 -m court.cli model master_of_coin)" --dir <wt>`). **Broad authority**: it may run live verification commands directly in the worktree to confirm Tribute claims, and it repairs/settles the Charter's paperwork. **Narrow remit**: it does no coding work — no code fixes, no touching the diff. Its job ends at exactly one Pass/Fail verdict plus Recommended Next Steps. **Commutation duty**: every audit names what production still needs to do to activate value (deploy commands, backfills, env var flips, migrations) under `Commutation`. **Sync-back**: the Master of Coin must always push its verdict back to the real branch (`git push . HEAD:<real-branch>`) before reporting done.
- **Gatekeeper on `the-gatehouse/<cogship_id>`**: The Gatekeeper runs inside a **brand-new ephemeral gatehouse convoy worktree** per Cog Ship convoy of size > 1 (or directly in the Quest worktree for size 1) via Kilo CLI (`python3 -m court.cli collect --standup` or `kilo run --agent gatekeeper --model "$(python3 -m court.cli model gatekeeper)" --dir <gatehouse_wt>`). Model: pinned in `.court/config.json` (`models.gatekeeper` / `models.master_of_coin`). Merges candidate branches, executes the unified integration test suite across the pack, isolates/rejects failing Quests via `/reject_tribute` with Serf remediation, promotes verified convoys directly into `castle`, and packs the Cog Ship manifest (`python3 -m court.cli ship`).
- **Goad on Stalled Sessions**: When a Serf stalls or goes idle before completing its checklist, goad it via Kilo CLI (`python3 -m court.cli goad <id>`), which launches a resumption turn under `--agent serf` with `goad_prompt.md`.
...
- **Steward on `castle`**: Manages lifecycle, planning, Council (`/plot`), dispatching, triage, and teardowns. Does not run test suites directly on `castle`. Model: pinned in `.court/config.json` (`models.steward`).
...
1. **Gate 1 — Creation & Spawning**:
   - Stand up Serf session via Court CLI (`court dispatch <id>`) or Kilo CLI (`kilo run --agent serf --model "$(python3 -m court.cli model serf)" --dir <wt>`).
...
3. **Gate 3 — Master of Coin Review (`TRIBUTE_READY`)**:
   - Dedicated session in worktree via `court coin <id>`. Verify clean tree and `behind: 0`. Audit line-by-line against Expected Tribute. Settle paperwork, name Commutation, advance to `GATE`, and sync back with `git push . HEAD:<branch>`.
...
4. **Gate 4 — Gatekeeper Integration (`GATE`)**:
   - Run unified integration tests on convoy branch via `court collect --standup` or Kilo CLI Gatekeeper, verify zero roleplay leakage in production assets, promote to `castle`, advance Quests to `READY_TO_RAZE`, and pack deployment manifest (`court ship`).
