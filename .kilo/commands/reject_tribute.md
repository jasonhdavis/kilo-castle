...
2. **Record the Rejection in the Quest's Charter:**
   ```bash
   court set-section <id> "Gatekeeper Review" --content "### ❌ Gatekeeper Rejection (Cog Ship Integration Failure)\n- **Failing Test Command**: `<cmd>`\n- **Error Traceback**:\n\`\`\`\n<traceback>\n\`\`\`\n- **Root Cause / Regression**: <details>\n- **Required Remediation**: <actionable steps>"
   court advance <id> WORKING --note "Gatekeeper rejected tribute: <one-line reason> (remediation in same worktree)"
   ```
